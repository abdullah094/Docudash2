import { StyleSheet, Text, View,Dimensions, TextInput,TouchableWithoutFeedback,Keyboard, Image,Pressable } from 'react-native'
...
{/* Header */}
      <View style={[{paddingVertical:20,zIndex:999,width:width,alignItems:'center',justifyContent:'center',position:'absolute',backgroundColor:Colors.green,flexGrow:1,top:0}]}>
        {/* Location show */}
        <View style={{top:10,width:'90%',}}>
        <EvilIcons name='location' size={35} color={Colors.white}/>
    

        <Text  style={{fontFamily:'Montserrat-SemiBold',color:Colors.white,fontSize:25,marginTop:10}} numberOfLines={1}>Your Nearest Notaries</Text>
       ...
       renderRow={(rowData) => {
        ...
        return (
         <View style={{height:18}}>
          <Text style={{ fontSize: 13,color:Colors.black }}>{title} {address}</Text>
  ...
  />
    ...
    >
     ...
     {data.map((element)=>{
      return(
       <Marker key={element.lat} onCalloutPress= {()=>navigation.navigate('Profile',{name:element.name})}  title={element.name} pinColor={Colors.darkgreen}  description={"Click to go to Profile"}  coordinate={{ latitude : element.long , longitude : element.lat }}>
       <Image style={{width:50,height:60}} source={require('../assets/images/pin/location-pointer-2.png')}/>
       </Marker>
      )
       })
}
